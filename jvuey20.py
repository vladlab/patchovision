...
class JellyfinSelector(App):
    ...
    def _navigate_to_seasons(self, series: dict) -> None:
        ...
        try:
            ...
            self.navigation_stack.append({"view": "library", "title": "Jellyfin Selector"})
    ...
    def _play_item(self, item: dict) -> None:
        ...
        self.exit((stream_url, self.drm_connector, self.drm_mode, self.audio_device))
    ...
    def _play_with_tracks(self) -> None:
        ...
        self.exit((stream_url, drm_connector, drm_mode, audio_device, track_args))
    ...
    def action_play(self) -> None:
        ...
        if self.current_view == "details":
            ...
            self._play_with_tracks()
        elif self.current_view not in ("settings", "settings_drm", "settings_drm_modes", "settings_audio"):
...
def get_credentials() -> tuple[str, str, str, str, str, str]:
    ...
    return server_url, api_key, user_id, drm_connector, drm_mode, audio_device
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("--setup", "--auth"):
        ...
        drm_connector, drm_mode, audio_device = "", "", ""
    else:
        server_url, api_key, user_id, drm_connector, drm_mode, audio_device = get_credentials()
    ...
    # Run app
    app = JellyfinSelector(server_url, api_key, user_id, drm_connector, drm_mode, audio_device)
    result = app.run()
    
    if result:
        # Unpack result
        if len(result) == 5:
            stream_url, drm_connector, drm_mode, audio_device, track_args = result
        else:
            stream_url, drm_connector, drm_mode, audio_device = result
            track_args = []
